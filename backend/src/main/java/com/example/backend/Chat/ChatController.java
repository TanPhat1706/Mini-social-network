...
public class ChatController {
    ...
    public ResponseEntity<List<ChatConversationDTO>> getRecentConversations() {
        ...
        // Lấy danh sách thô từ Database
        List<ChatConversationDTO> rawConversations = chatMessageRepository.findRecentMessages(userId);

        // 🟢 CẬP NHẬT LOGIC LỌC BỎ NHỮNG PARTNER KHÔNG TỒN TẠI
        List<ChatConversationDTO> validConversations = new ArrayList<>();
        for (ChatConversationDTO conv : rawConversations) {
            // Kiểm tra xem partner có còn tồn tại trong DB không
            boolean isPartnerExists = userRepository.findById(conv.getPartnerId()).isPresent();
        ...
        return ResponseEntity.ok(validConversations);
    }
